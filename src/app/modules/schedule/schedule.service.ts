    ...
    schedule.weeks.forEach((week) => {
      schedule.weeks.forEach((week) => {
        week.sessions.forEach((session) => {
          ...
          totalEnrollment += session.totalRegistered || 0;
        });
      });
    });
...
const updateSchedule = async (id: string, data: any) => {
  ...
  if (weeks && weeks.length > 0) {
    ...
    const sortedNewWeeks = [...weeks].sort(
      (a: any, b: any) => a.weekNumber - b.weekNumber,
    );
    updateData.startDate = new Date(sortedNewWeeks[0].startDate);
    updateData.endDate = new Date(
      sortedNewWeeks[sortedNewWeeks.length - 1].endDate,
    );
...
const updateWeekCapacity = async (weekId: string, capacity: number) => {
  ...
  await CacheInvalidator.onRecordUpdate(
    "schedulePeriod",
    week.schedulePeriodId,
  );
